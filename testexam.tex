...
\frac{\newd^n}{\newd x^n}[f(x)g(x)] = \sum\limits_{i=0}^{n}\binom{n}{i}f^{(i)}(x)g^{(n-i)}(x).
\end{equation}
...
\end{theorem}
\begin{equation}
\frac{1}{2\pi \newi} \int\limits_{\gamma}f = \sum\limits_{k=1}^{m}n(\gamma;a_{k})Res(f;a_k).
\end{equation}
...
\Gamma(z)\equiv \lim\limits_{n \to \infty} \frac{(n+1)^z n!}{z(z+1) \cdots (z+n)} \equiv \int_{0}^{\infty} e^{-t}t^{z-1} \, \newd t.
\end{equation}
